...
export const NodeStatusIndicator = ({
  ...
  switch (status) {
    ...
    case "success":
      return (
        <StatusBorder className={cn("border-green-700/50", className)}>{children}</StatusBorder> // {children}</StatusBorder> // <StatusBorder className="border-emerald-600"}>{children}</StatusBorder>
      );
    case "error":
      return <StatusBorder className={cn("border-red-700/50", className)}>{children}</StatusBorder>; // <StatusBorder className="border-red-400"}>{children}</StatusBorder>;
    default:
      return <>{children}</>;
  }
};
